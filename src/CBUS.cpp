...
void CBUS::CANenumeration(void) {
  ...
  // initiate CAN bus enumeration cycle, either due to ENUM opcode, user button press, change of FLiM state, or clash detection
...
void CBUS::setLEDs(CBUSLED green, CBUSLED yellow) {
	UI = 1;
	Serial << F("> setting CBUS LEDs") << endl;
if (UI) {
  _ledGrn = green;
  _ledYlw = yellow;
}
...
void CBUS::setSwitch(CBUSSwitch sw) {
	UI = 1;
	Serial << F("> setting CBUS Switch") << endl;
if (UI) {
  _sw = sw;
}
}
...
//  bool enumeration_required = false;
  byte remoteCANID, nvindex, nvval, evnum, evindex, evval, tarray[4], opc;
  unsigned int nn, en;
...
if (UI) {
    ...
    if (bCANenum && !bCANenumComplete) {
      ...
      if (_msg.len == 0) {
        ...
        // store this response in the array
        if (remoteCANID > 0) {
    ...
    case OPC_ASOF3:
      ...
      // accessory on or off
      // try to find a matching stored event -- match on nn and en
    ...
    case OPC_RQNP:
      // RQNP message - request for node paramters -- only respond if we are in transition to FLiM
...
if (UI) {
    ...
    case OPC_EVLRN:
      ...
      if (bLearn == true) {
        ...
        // not found - it's a new event so try to find space for it
        if (index >= config.EE_MAX_EVENTS) {
        ...
        if (index < config.EE_MAX_EVENTS) {
          ...
          // no need to repeat this for subsequent EVs
          if (evindex < 2) {
            // tarray[0] = _msg.data[1];
            // tarray[1] = _msg.data[2];
            // tarray[2] = _msg.data[3];
            // tarray[3] = _msg.data[4];
            memcpy(&tarray[0], &_msg.data[1], 4);
          ...
          // Serial << F("> no free event space, index = ") << index << endl;
          // respond with CMDERR
          sendCMDERR(10);
        }
    ...
    case OPC_PNN:
      // another node responding to QNN
      break;
